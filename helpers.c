...
void blur(int height, int width, RGBTRIPLE image[height][width])
{
    float average;
    int i, j;
    int sum; //to add the color values of the neighbouring pixels
    ...
    for (i = 0; i < height; i++)
    {
        ...
        {
            //Modify the red value paying attention not to go over the image limits
            sum = image[i][j].rgbtRed;
            count = 1;
            if (i > 0)
            {
            ...
            image[i][j].rgbtRed = round(average); //since the value should be an integer
