...
void add_pairs(void)
{
    int count;
    int i, j;
    int max; //maximum number of pairs possible according to the number of candidates
    //from 0 to pair_count - 1
    max = candidate_count * (candidate_count - 1) / 2;
    ...
    count = 0;
    for (i = 0; i < candidate_count - 1 && count < max; i++)
    {
        for (j = 0; j < candidate_count && count < max; j++)
        {
...
void sort_pairs(void)
{
    ...
    {
        cur = preferences[pairs[i].winner][pairs[i].loser];
        for (j = i + 1; j < pair_count; j++)
        {
            next = preferences[pairs[j].winner][pairs[j].loser];
            if (next > cur)
            {
                tmp = pairs[i];
                pairs[i] = pairs[j];
...
void lock_pairs(void)
{
    ...
    for (i = 0; i < pair_count; i++)
    {
        ...
        if (locked[winner][loser] != true)
        {
            ...
            pointed[loser] ++;
            iscircle = 1;
            for (j = 0; j < candidate_count && iscircle == 1; j++)
            {
                if (pointed[j] == 0)
                {
                    iscircle = 0;
            ...
            /*
            ris = thereiscircle();
            if (ris == 1) //returns 1 if there is a circle
            {
                locked[winner][loser] = false;
            } */
        }
...
/* int thereiscircle (void)
{
    int isOK;
    int i, j;
    int count; //counts the number of true in each column

    isOK = 1;
    for (j = 0; j < pair_count && isOK ==1; j++)
    {
        count = 0;
        for (i=0; i < pair_count; i ++)
        {
